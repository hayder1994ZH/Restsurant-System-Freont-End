                ...
                <b-tr v-for="(item, index) in items" :key="index">
                  ...
                  <b-td
                    v-if="
                      ...
                      item.video_object.resolution_url
                    "
                      ...
                      <div class="mr-2 d-flex">
                        <b-button
                          ...
                          v-if="
                            item.status === 0 &&
                            item.video_object &&
                            item.video_object.resolution_url.length
                          "
...
export default {
  ...
  methods: {
    ...
    videoPopup (id) {
      this.videoSource = []
      this.axios.get(`video/${id}`).then((res) => {
        if (res.data.video_object == null) {
          this.$swal(
            'خطأ في التشغيل',
        ...
        if (res.data.video_object.resolution_url[0]) {
          res.data.video_object.resolution_url.forEach((video) => {
    ...
    getAllVideo () {
      ...
      this.axios
        .get(
          `video?take=${this.perPage}&skip=${pageSkip}&filter[title]=${this.search}&filter[gender_id]=${this.searchGender}&filter[subcategory_id]=${this.searchSubcategory}`
        )
        .then((res) => {
