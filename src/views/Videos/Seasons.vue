    ...
    <b-row>
      <b-col sm="12">
        ...
        <b-modal id="addSeasonModal" hide-footer>
          <div class="mb-3">
            <label
              ><span style="color: red">* </span> Enter Season Number</label
            >
            <b-form-input
              type="number"
      ...
      <b-col sm="12">
        <iq-card>
          <template v-slot:headerTitle>
            <h4 class="card-title">Seasons List</h4>
          </template>
...
export default {
  ...
  methods: {
    ...
    getAllSeasons () {
      ...
      this.axios
        .get(
          `season?take=${this.perPage}&skip=${pageSkip}&filter[number]=${this.search}`
        )
        .then((res) => {
        ...
        this.axios
          ...
          .catch((res) => {
            if (
              ...
              this.$swal('الموسم مكرر', 'يرجى اختيار رقم غير مكرر', 'error')
              return ''
            } else {
    ...
    editSeason () {
      ...
      this.axios.post(`season/${this.formEdit.id}`, formData).then(() => {
        this.getAllSeasons()
