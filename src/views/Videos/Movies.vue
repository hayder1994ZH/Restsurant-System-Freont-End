...
export default {
  ...
  methods: {
    ...
    videoPopup (id) {
      this.videoSource = []
      this.axios.get(`movie/${id}`).then((res) => {
    ...
    getAllMovie () {
      ...
      this.axios
        .get(
          `movie/all/production?take=${this.perPage}&skip=${pageSkip}&filter[subcategory_id]=5&filter[title]=${this.search}`
        )
        .then((res) => {
