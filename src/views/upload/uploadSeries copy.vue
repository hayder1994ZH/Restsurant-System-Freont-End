<template>
  <b-container fluid>
    <b-row>
      ...
      <b-col sm="12" v-for="(_, index) in videoFiles" :key="index">
        <b-card no-body>
              ...
              <b-form v-show="isInfoUploaded">
                <b-row>
                  <b-col sm="12" md="8">
                    ...
                    <b-form-group
                      id="input-group-3"
                      label="Choose Eposide:"
                      label-for="input-3"
                    >
                      <b-form-select
                        id="input-3"
                        v-model="videoFiles[index].eposide"
                        :options="eposides"
                        required
                      ></b-form-select>
                    </b-form-group>
                    <b-form-group label="Eposide File">
                      <b-form-file
...
export default {
  ...
  methods: {
    ...
    manageUpload () {
      ...
      if (!this.season) {
        this.$swal('خطأ في الادخال', 'يرجى اختيار الموسم', 'error')
        return '400'
      }

      for (let i = 0; i < this.videoFiles.length; i++) {
        if (!this.videoFiles[i].eposide) {
          this.$swal('خطأ في الادخال', 'يرجى اختيار الحلقة', 'error')
          return '400'
        }
    ...
    formDataq (index) {
      ...
      formData.append('season_id', this.season)
      formData.append('eposide_id', this.videoFiles[index].eposide)
      if (this.count === 1) {
    ...
    async uploadChunks () {
      ...
      await this.axios
        .post(
          `movie/upload/chunk/${this.videoInfoIds[0].videoable_id}`,
